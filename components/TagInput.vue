  ...
  <div class="max-w-3xl">
    <div class="flex flex-wrap items-center justify-start">
      <div v-for="(tag, index) in tags" :key="tag" class="space-x-2">
        <div
          class="flex items-center h-5 p-1 m-1 border-2 border-cyan-600 rounded-lg "
        >
          <div class="text-[12px] px-2 font-bold text-cyan-600 border-cyan-600">{{ tag }}</div>
          <svg
            @click="removeTag(tag)"
      ...
      <input
        ...
        class="w-20 m-1 text-xs font-semibold text-center text-cyan-600 bg-transparent border-2 rounded-full focus:outline-none border-stone-300 placeholder-stone-500 focus-within:border-cyan-600 "
        v-model="newtag"
        @keydown="addTag"
      />
      ...
      >
        <div
          ...
          cursor-pointer
            ...
            rounded-full
            border-gray-500
            mt-2 hover:bg-cyan-600 hover:text-cyan-100
          "
        >
          {{ tag }}
        </div>
